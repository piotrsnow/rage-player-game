...
- currentLocation: emit ONLY when the player ARRIVES at a different location THIS scene. Value is the EXACT canonical name from the [TRAVEL] block / sublocation entry / [DUNGEON ROOM] exits. NEVER invent a name. NEVER set currentLocation to a place the player is LEAVING. Polish "z [lokacji]" / "wychodząc z" / "opuszczam" means DEPARTURE — the destination is the PARENT or OUTSIDE, never the named source. If the player mentions a location only as origin, emit the parent location name or omit.
