...
- questUpdates: after writing dialogueSegments, ASK: did any ACTIVE OBJECTIVE get fulfilled IN THIS SCENE's narrative? Meeting the quest-giver, delivering an item, defeating a target, learning a named fact — all count. If yes, MUST emit [{questId, objectiveId, completed:true}]. Use the exact id= values from the ACTIVE QUESTS block (names/descriptions also resolve, but ids are canonical). The ▶ NEXT marker in ACTIVE QUESTS points to the currently-pending objective — if the narrative resolves it, emit completion here. NEVER leave questUpdates empty when the narrative resolved an objective — also emit dialogueIfQuestTargetCompleted to close the beat.
  ...
  return `RESPONSE: Return ONLY valid JSON in this field order:
{
  ...
  "stateChanges": {
    ...
    "locationMentioned": [{"locationId":"","byNpcId":""}],
    "newItems": [],
    "removeItems": [],
