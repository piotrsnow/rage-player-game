  ...
  for (const rawTalent of talents) {
    ...
    if (effect.skills && skillName) {
      ...
      applicable = effect.skills.some((s) => {
        if (s.endsWith('(Any)')) {
          const prefix = s.replace(/\s*\(Any\)/, '').trim();
          return normalizedSkill === prefix || skillName.startsWith(prefix);
        }
    ...
    if (!applicable && effect.characteristic === characteristicKey && !effect.skills) {
      applicable = true;
    }
