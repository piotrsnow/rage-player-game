...
window.DICE = (function() {
    ...
    function apply_theme_options(options) {
        ...
        if (options.labelColor != undefined) vars.label_color = options.labelColor;
        if (options.diceColor != undefined) vars.dice_color = options.diceColor;
        if (options.ambientLightColor != undefined) vars.ambient_light_color = options.ambientLightColor;
        if (options.ambientLightIntensity != undefined) vars.ambient_light_intensity = options.ambientLightIntensity;
        if (options.spotLightColor != undefined) vars.spot_light_color = options.spotLightColor;
        if (options.spotLightIntensity != undefined) vars.spot_light_intensity = options.spotLightIntensity;
    ...
    that.dice_box = function(container, options) {
        ...
        this.scaleMultiplier = (typeof options.scaleMultiplier === 'number' && options.scaleMultiplier > 0)
            ? options.scaleMultiplier : 1;
        this.hideDeskVisual = Boolean(options.hideDeskVisual);
    ...
    that.dice_box.prototype.reinit = function(container) {
        ...
        this.light = null;
        if (!vars.disable_spot_light) {
            this.light = new THREE.SpotLight(vars.spot_light_color, vars.spot_light_intensity);
        ...
        this.desk = new THREE.Mesh(new THREE.PlaneGeometry(this.w * 2, this.h * 2, 1, 1), 
                new THREE.MeshPhongMaterial({
                    ...
                    opacity: this.hideDeskVisual ? 0 : vars.desk_opacity,
                    transparent: true
                }));
        this.desk.receiveShadow = this.hideDeskVisual ? false : vars.use_shadows;
        this.desk.visible = !this.hideDeskVisual;
