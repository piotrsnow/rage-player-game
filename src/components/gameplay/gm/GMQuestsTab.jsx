  ...
  if (active.length === 0 && completed.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3 text-outline">
        ...
        <p className="text-sm font-label uppercase tracking-widest">{t('gmModal.emptyQuests')}</p>
      </div>
    );
  }
    ...
    <div className="h-full overflow-y-auto custom-scrollbar p-4 space-y-6">
      ...
      {active.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-outline">({active.length})</span>
          </div>
      ...
      {completed.length > 0 && (
        <div>
          ...
          >
            <span className="material-symbols-outlined text-base text-outline transition-transform group-hover:text-on-surface-variant"
              style={{ transform: showCompleted ? 'rotate(90deg)' : 'rotate(0deg)' }}
            >
              chevron_right
            </span>
            <span className="text-xs font-label uppercase tracking-widest text-outline group-hover:text-on-surface-variant transition-colors">
              {t('gmModal.completedQuests')}
            </span>
            <span className="text-xs text-outline">({completed.length})</span>
          </button>
          {showCompleted && (
            <div className="space-y-3">
              {completed.map((quest) => (
    ...
    <div className={`p-3 rounded-sm border ${
      ...
      <div className="flex items-center justify-between mb-1.5">
        <div className="flex items-center gap-2 min-w-0">
          <span className={`material-symbols-outlined text-base ${readyToTurnIn ? 'text-amber-400' : isActive ? 'text-primary' : 'text-outline'}`}>
            {readyToTurnIn ? 'assignment_return' : isActive ? 'assignment' : 'assignment_turned_in'}
          </span>
          <span className="text-sm font-bold text-on-surface truncate">{quest.name}</span>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {readyToTurnIn && (
            <span className="text-[9px] font-label uppercase tracking-wider text-amber-400 bg-amber-500/15 px-1 py-0.5 rounded-sm">
      ...
      {quest.description && (
        <p className="text-xs text-on-surface-variant mb-2">{quest.description}</p>
      )}
      ...
      {rewardText && (
        <div className="flex items-center gap-1.5 mb-2 text-xs">
          <span className="material-symbols-outlined text-sm text-amber-400">paid</span>
      ...
      {/* Metadata */}
      <div className="flex items-center gap-3 flex-wrap text-[10px] text-outline">
        {giver && (
