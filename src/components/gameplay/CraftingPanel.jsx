...
export default function CraftingPanel({ character, dispatch, disabled }) {
  const { t } = useTranslation();
  const [selectedRecipe, setSelectedRecipe] = useState(null);
  const [lastResult, setLastResult] = useState(null);

  const recipes = useMemo(() => {
    const allRecipes = gameData.craftingRecipes || [];
    return getAvailableRecipes(character?.materialBag || [], character?.skills || {}, allRecipes);
  }, [character?.materialBag, character?.skills]);

  const rzemiosloLevel = getSkillLevel(character?.skills, 'Rzemioslo');
    ...
    <div className="bg-surface-container-low/60 backdrop-blur-sm border border-tertiary/20 rounded-sm p-3 space-y-3 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="material-symbols-outlined text-sm text-tertiary">construction</span>
          <span className="text-[10px] font-label font-bold uppercase tracking-widest text-on-surface">
            {t('crafting.title')}
          </span>
          <span className="text-[9px] text-on-surface-variant">
            {t('crafting.skillLevel', { level: rzemiosloLevel })} | INT: {inteligencja}
          </span>
        </div>
        <button
          onClick={handleClose}
      ...
      <div className="space-y-1.5 max-h-72 overflow-y-auto custom-scrollbar">
        {recipes.length === 0 && (
          <p className="text-[10px] text-on-surface-variant text-center py-4">{t('crafting.noRecipes')}</p>
        )}
        {recipes.map((recipe, idx) => {
          ...
          return (
            ...
            >
              ...
              {/* Material status (always visible for selected, compact for others) */}
              {isSelected && (
                <div className="mt-2 space-y-1">
                  ...
                  <button
                    ...
                    className="mt-2 w-full py-1.5 text-[10px] font-label font-bold uppercase tracking-widest rounded-sm bg-primary/20 border border-primary/30 text-primary hover:bg-primary/30 disabled:opacity-40 transition-colors"
                  >
                    {t('crafting.craft')}
                  </button>
                </div>
              ...
              {/* Compact material summary when not selected */}
              {!isSelected && (
      ...
      {lastResult && (
        <div className={`text-[10px] p-2 rounded-sm border ${
          lastResult.success
            ? lastResult.tier === CRAFTING_TIERS.CRITICAL_SUCCESS
              ? 'bg-tertiary/10 border-tertiary/20 text-tertiary'
              : 'bg-success/10 border-success/20 text-success'
            : 'bg-error/10 border-error/20 text-error'
        }`}>
          {lastResult.tier === CRAFTING_TIERS.CRITICAL_SUCCESS && t('crafting.criticalSuccess')}
          {lastResult.tier === CRAFTING_TIERS.SUCCESS && t('crafting.success', { item: lastResult.resultItem?.name })}
          {lastResult.tier === CRAFTING_TIERS.PARTIAL_FAILURE && t('crafting.partialFail')}
          {lastResult.tier === CRAFTING_TIERS.CRITICAL_FAILURE && t('crafting.criticalFail')}
          {lastResult.skillCheck && (
            <span className="block mt-1 text-[9px] text-on-surface-variant">
              d50: {lastResult.skillCheck.roll} | {t('crafting.difficulty')}: {lastResult.skillCheck.threshold} | margin: {lastResult.skillCheck.margin > 0 ? '+' : ''}{lastResult.skillCheck.margin}
            </span>
          )}
        </div>
      )}
