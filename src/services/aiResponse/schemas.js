...
const StateChangesSchema = z.object({
  ...
  learnSpellSchool: z.string().nullable().optional(),
  learnSpellManaCost: z.number().int().min(1).max(10).nullable().optional(),
  learnSpellDescription: z.string().max(500).nullable().optional(),
  learnSpellLongDescription: z.string().max(1000).nullable().optional(),
  learnCustomSpellId: z.string().uuid().nullable().optional(),
  ...
  characterEffects: z.array(z.object({
    action: z.enum(['add', 'remove', 'tick']),
    target: z.string().optional().default('player'),
    name: z.string(),
    effectId: z.string().optional(),
    effect: z.object({
      id: z.string().min(1),
      name: z.string().min(1),
      source: z.enum(['spell', 'item', 'combat', 'trap', 'environmental', 'ai']).optional().default('ai'),
      category: z.enum(['buff', 'debuff', 'dot', 'control', 'mixed']).optional().default('debuff'),
      duration: z.object({
        type: z.enum(['rounds', 'scenes', 'time', 'permanent', 'until_rest', 'manual']),
