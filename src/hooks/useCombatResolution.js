...
export function soloPerCharForServer(perChar) {
  const out = {};
...
/**
 * Pure factory for combat-ending handlers. Kept outside the hook so it can
 * be unit-tested without rendering a React tree.
 */
export function buildCombatResolutionHandlers({
  isMultiplayer,
  dispatch,
  autoSave,
  narrator,
...
export function useCombatResolution({
  ...
  return useMemo(
    () => buildCombatResolutionHandlers({ isMultiplayer, dispatch, autoSave, narrator, generateScene, mp, settings, t }),
