...
- **Trace, gdy szeroki audyt nic nie daje.** Jeśli regression dalej żyje po breadth-first audycie pól / typów / migracji, albo coś wydaje się nie zgrywać między `knowledge/` a kodem — przełącz się na end-to-end flow trace **konkretnej** user-journey (jeden POST/SSE/render od początku do końca, plik po pliku). Ustal prawdziwe źródło prawdy w kodzie i poinformuj użytkownika o znalezisku zanim cokolwiek zaproponujesz. Powierzchowny audyt łapie strukturalne problemy; bug na seam'ie między dwoma requestami / FE↔BE filtrem łapie tylko trace.
...
Persistent files: `CLAUDE.md` (this file), `knowledge/` (subsystem deep-dives, decisions, patterns, ideas), and per-conversation memory. Before touching a subsystem, YOU MUST read its `knowledge/concepts/*.md` file — it exists because the answer wasn't obvious from the code. After learning something durable, write it down where it belongs; code rot is faster than you think.
...
- **Services** — AI dispatch (`src/services/ai/`), AI response parsing (`src/services/aiResponse/`), game engines (combat/magic/trade/crafting/alchemy), deterministic mechanics (`mechanics/`), storage/sync (`storage.js`), API client (`apiClient.js`).
- **Lokalizacje** — **Location Graph jest jedynym systemem** lokacji (player, NPC, świat). Frontend: `src/components/gameplay/locationGraph/*` + `useLocationGraph` hook + `useCurrentLocationNode` selector. State: `world.currentLocationRef: { kind, id } | null` i `npc.locationRef`. Helpery: `src/utils/locationRef.js`. Schemas: `shared/domain/locationGraph.js`, `shared/domain/tacticalGrid.js`. Wywalone (Faza 5/6): `fieldMap/`, `MapCanvas`, `PlayerWorldMap`, `MapTab`, `mapSprites`, `useCampaignMap`.
...
- **Living World** — `services/livingWorld/` — campaign sandbox (CampaignNPC shadow + F5b CampaignLocation per-campaign sandbox), `questGoalAssigner/` (slim — `pickQuestGiver` Phase D + `categorize`; folder name vestigial after BE goal-assigner archive → [knowledge/ideas/npc-action-assignment.md](knowledge/ideas/npc-action-assignment.md)), npc agent loop, reputation, travel graph (Dijkstra over canonical-only `Road`s — DEPRECATED, migracja do `LocationEdge` planowana), dungeon seed, fog-of-war discovery helpers, hearsay orchestration. AI mid-play creation lands in `CampaignLocation`; canonical promotion happens via admin queue.
- **Location Graph (BE)** — `services/locationGraph/` — `graphService` (loadSubgraph + node CRUD + helpery: `getNodeByRef`, `bumpVisitCount`, `appendModificationLog`, `recordNpcEncounter`, `markLiberated`, `setDungeonState`), `graphContextBuilder` (AI prompt context z 1-hop neighbors), `graphExtractor` (parse AI graphUpdate), `graphValidator` (Zod + consistency check), `aiResolver` (string-name → composite ref + `createNodeFromAIProposal`), `movementEngine`, `knowledgeFilter` (per-campaign fog-of-war filter), `seedEdges`. Polymorphic kind+id: `world` (canonical `WorldLocation`) vs `campaign` (sandbox `CampaignLocation`). Edges: `LocationEdge` (43 typy w 8 kategoriach: structural/spatial/movement/access/perception/social/narrative/temporal).
...
| `CampaignQuickBeat` | "Mała akcja" — lightweight RP-beat anchored to a `CampaignScene` via `parentSceneIndex`. Bypasses scene-gen pipeline (no embedding / memory compression / Living World ticks). FE caps streak at 5 between full scenes. See [knowledge/concepts/quick-beats.md](knowledge/concepts/quick-beats.md). |
...
- `knowledge/concepts/biome-and-step-movement.md` — polygon biome map (`getBiomeForCoords`), free-vector movement, pathScan + 250 m discovery, world barriers, anchored vs wandering Campaign position
- `knowledge/concepts/quick-beats.md` — "mała akcja" lightweight RP-beat path (nano single-shot, no scene row, no post-scene work), FE streak/lock, escalation guard, `[RECENT QUICK BEATS]` prompt continuity
...
Current ideas: async-tool-pattern, autonomous-npcs, combat-auto-resolve, declarative-event-routing, deferred-event-batching, living-world-admin-extras, living-world-atonement-loop, living-world-cross-user-visibility, living-world-npc-auto-dispatch, living-world-scene-orchestration, living-world-vector-search, npc-action-assignment, prompt-fragment-system, side-quests-between-campaigns. See [knowledge/ideas/README.md](knowledge/ideas/README.md).
...
| Polymorphic location lookup (F5b) | [backend/src/services/livingWorld/worldStateService.js](backend/src/services/livingWorld/worldStateService.js) `resolveLocationByName` (canonical-first, falls back to this-campaign CampaignLocation) + `findOrCreateCampaignLocation` (per-campaign sandbox creation). `resolveWorldLocation` is canonical-only LOOKUP — never creates (former `findOrCreateWorldLocation` had a fallback create-at-(0,0) branch that produced rogue WorldLocations overlapping the capital; deleted). |
| Quest-giver picker (Phase D) | [backend/src/services/livingWorld/questGoalAssigner/npcGiverPicker.js](backend/src/services/livingWorld/questGoalAssigner/npcGiverPicker.js) `pickQuestGiver` — used by `generateSceneStream` when nano flags `quest_offer_likely`. (Campaign-side activeGoal mechanic archived → [knowledge/ideas/npc-action-assignment.md](knowledge/ideas/npc-action-assignment.md).) |
...
| Biome map (polygons + lookup) | [shared/domain/biomeMap.js](shared/domain/biomeMap.js) `getBiomeForCoords` — Bezier paths sampled at module load, point-in-polygon. SVG editing surface: [plans/biome-tiles-proposal.svg](plans/biome-tiles-proposal.svg) |
| Step movement: vector intent | [shared/domain/movementIntent.js](shared/domain/movementIntent.js) `parseMovementIntent`, `applyMovementVector` |
| Step movement: path scan | [backend/src/services/livingWorld/pathScan.js](backend/src/services/livingWorld/pathScan.js) `scanPath` (DB) + `computePathScan` (pure) — POIs within 250 m, biome transitions, sorted by `alongKm` |
| World-edge barriers | [shared/domain/worldBarriers.js](shared/domain/worldBarriers.js) — N/S kopiące robaki, W smok, E ocean; surfaced in `[WORLD BOUNDS]` + `[MOVEMENT].barrierHit` |
...
| Quick beats ("mała akcja") | [backend/src/services/sceneGenerator/quickBeat.js](backend/src/services/sceneGenerator/quickBeat.js) `runQuickBeat` + `shouldEscalateQuickBeat` (nano single-shot, NPC whitelist, escalation regex). Route: [backend/src/routes/ai/quickBeatStream.js](backend/src/routes/ai/quickBeatStream.js). FE: [src/hooks/sceneGeneration/useQuickBeat.js](src/hooks/sceneGeneration/useQuickBeat.js) + `ADD_QUICK_BEAT` in [src/stores/handlers/sceneHandlers.js](src/stores/handlers/sceneHandlers.js). Persisted in `CampaignQuickBeat`; hydrated by [src/services/storage/campaignParse.js](src/services/storage/campaignParse.js) (interleaved into `chatHistory`, streak rebuilt). Premium continuity via `recentQuickBeats` → `[RECENT QUICK BEATS]` block in [userPrompt.js](backend/src/services/sceneGenerator/userPrompt.js). Solo-only V1; MP deferred. See [knowledge/concepts/quick-beats.md](knowledge/concepts/quick-beats.md). |
...
- **In-memory rate limit + idempotency require single instance.** The rate-limit counters (in-memory Map in `@fastify/rate-limit`) and the idempotency store (`Map()` in `plugins/idempotency.js`) are not shared across processes. Cloud Run must stay at `max_instances: 1` until these are migrated to a shared store (Postgres table or Redis). If multi-instance scaling becomes necessary, implement a Postgres-backed idempotency table (`IdempotencyEntry`: key, status, response, expiresAt with `ON CONFLICT` upsert) and a shared rate-limit store.
...
- **Living World settlement seeding (Phase A/B).** Every `livingWorldEnabled` campaign gets hamlets/villages/towns/cities pre-seeded at POST `/v1/campaigns` (counts scale with `length`, `Campaign.worldBounds` centered on global capital Yeralden). **All top-level location creation is blocked mid-play** — `processLocationChanges` silently rejects every `parentLocationName=null` entry regardless of locationType (settlements, wilderness, dungeons, ruins — everything). AI dungeons mid-play also blocked (no path in `processStateChanges`). Sublocations (`parentLocationName` set) remain creatable as `CampaignLocation` per-campaign sandbox without a hard cap (`topologyGuard.decideSublocationAdmission` only rejects on missing/generic name). See [knowledge/concepts/living-world.md](knowledge/concepts/living-world.md).
- **Travel + step movement = unified match-or-drop + pathScan + barriers (F5d).** Player text routes through three intent paths: (1) named-target ("idę do Kamionki", map click) → `_travelTarget` → resolved POI; (2) free-vector ("1 km na północ", distance required) → `_directionalMove` → `applyMovementVector(curX, curY, az, dist)`; (3) direction-only ("idę na N") → dungeon nav. All three converge on [`buildTravelBlock`](backend/src/services/aiContextTools/contextBuilders/travel.js) which runs [`scanPath`](backend/src/services/livingWorld/pathScan.js) — POIs within 250 m of the segment, biome transitions, destination-tile occupancy. `## [MOVEMENT]` block in [`contextSection.js`](backend/src/services/sceneGenerator/contextSection.js) instructs premium to narrate accordingly and emit `stateChanges.currentLocation` (anchored at POI) OR `stateChanges.currentX/Y` (wandering). [`processStateChanges/index.js`](backend/src/services/sceneGenerator/processStateChanges/index.js) handles three modes: name resolves → FK trio + sync coords from POI; name unresolved + coords → wandering with flavor name (no DB row); bare coords → wandering no flavor. **AI never creates locations**. Pass-by canonical POIs flip to `heard_about` automatically. World edges blocked by named barriers ([shared/domain/worldBarriers.js](shared/domain/worldBarriers.js)) — destination clamped, AI narrates obstacle. See [knowledge/concepts/biome-and-step-movement.md](knowledge/concepts/biome-and-step-movement.md).
...
- **Stable Diffusion defaults are tuned for DreamShaperXL Turbo v2.** [backend/src/config.js](backend/src/config.js) `sdWebui` ships with `sampler='DPM++ SDE Karras'`, `steps=6`, `cfg=2.0` — correct for SDXL-Turbo/LCM checkpoints. For a vanilla (non-Turbo) SDXL model set `SD_WEBUI_STEPS=25`, `SD_WEBUI_CFG=7`, `SD_WEBUI_SAMPLER="DPM++ 2M Karras"` in env, otherwise scene output will look undercooked. Scene dimensions default to a native SDXL 16:9 bucket (`SD_WEBUI_SCENE_WIDTH=1344`, `SD_WEBUI_SCENE_HEIGHT=768`). Opt-in hires fix: `SD_WEBUI_HIRES_FIX=1` (roughly doubles generation time but fixes blurry faces in wide shots). All are optional overrides — unset values use the code defaults. [docker-compose.yml](docker-compose.yml) forwards `SD_WEBUI_*` including URL, sampler, steps, cfg, hires fix, and scene width/height. Remember the three-file env checklist below if you set them.
- **Two `.env` files, two audiences — a new backend env var needs THREE edits.** The repo has root `.env` (consumed by `docker-compose.yml` via `${VAR}` interpolation) and `backend/.env` (consumed by `dotenv/config` in `backend/src/config.js` when the backend runs natively on the host, e.g. `prisma migrate`). The backend container does **not** mount `backend/.env` — it only sees env vars that `docker-compose.yml` **explicitly** forwards from the root `.env`. Adding a value to just one of these files is the #1 "I set the env var and it's still missing" trap. Checklist for any new backend env var:
  1. Add to root `.env` (and `.env.example` if it exists).
  2. Add to `backend/.env` (for host-side scripts + to keep the two in sync).
  3. Add to the `environment:` block of the `backend` service in `docker-compose.yml` as `VAR_NAME: "${VAR_NAME:-}"` — **without this line the container never sees it**.
  4. Restart the backend (`npm run dev:down && npm run dev:backend`). `dotenv` and compose only read env at process start — editing `.env` while the container runs does nothing.
  
  Symptom when step 3 is skipped: the variable works for `prisma`/scripts but the API reports it as "not configured" (e.g. `SD_WEBUI_URL` missing, `GEMINI_API_KEY` unavailable in the Keys modal even though it's in `backend/.env`).
